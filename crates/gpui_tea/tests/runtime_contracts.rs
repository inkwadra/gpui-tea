...
#[gpui::test]
fn composite_subscriptions_panic_when_rebased_keys_collide(cx: &mut TestAppContext) {
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    enum Msg {
        First(()),
        Second(()),
    ...
    impl Model for SubscriptionChild {
        ...
        fn subscriptions(
            ...
            Subscriptions::one(Subscription::new("clock", |_cx| SubHandle::None))
        }
    ...
    let result = cx.update(|cx| {
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || {
            let scope = ModelContext::root();
            model.__composite_subscriptions(cx, &scope)
        }))
    });
