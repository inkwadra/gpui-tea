...
fn main() {
    eprintln!(
        "Enable the `tracing` feature to run `RUST_LOG=debug cargo run --example telemetry --features tracing`."
    );
}
...
mod enabled {
    ...
    use gpui_tea::{
        Command, Dispatcher, IntoView, Model, Program, ProgramConfig, View,
